...
# Aurora, RDS Proxy) across 2 AZs. Lambda runs in the private subnets so it
# can reach RDS Proxy without traversing the internet; a single NAT gateway
# (not one per AZ) keeps cost down for this workload — flagged as a
# cross-AZ-NAT-traffic tradeoff, not a mistake, revisit if traffic grows.
